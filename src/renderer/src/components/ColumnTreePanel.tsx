...
import {ArrowDown, ArrowUp, Database, Folders, RefreshCcw, Save, Table2} from 'lucide-react'
...
function ColumnTreePanel() {
    ...
    const disableMoveActions = busy === 'applying' || busy === 'connecting' || busy === 'saving'
    const disableTargetSelection = !snapshot || !hasResolvedTarget || disableMoveActions
    const toolbarBusy = busy === 'connecting'
        ...
        <section className={cn(cardPanelClass, 'flex min-h-0 flex-col')}>
            ...
            <div className="border-b border-studio-border/80 px-3.5 py-3">
                {snapshot && snapshot.target ? (
                    ...
                    <div className="text-center text-sm text-studio-muted">No table selected.</div>
                )}
            </div>
